...
pub fn record_start()->Result<(),String>{
    ...
    let _producer_thread = thread::spawn(move ||{
        ...
        if let Err(_error) = acc_result_obj{
            return;
        }
    ...
    let _consumer_db_thread = thread::spawn(move ||{
        ...
        while run{
            ...
            if first {
                let _full_telemetry = telemetry.clone();
            }
            if counter == 0{
            ...
            let _id = db.iterate("SELECT last_insert_rowid();", |_row|->bool{
                return true;
            });
