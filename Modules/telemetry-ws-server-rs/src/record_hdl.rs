...
use crate::message_parser::commands::RecordParam;
...
pub fn record_start(param:RecordParam)->Result<(),String>{
    if param.game.eq("ACC") {
        ...
        let producer_thread = thread::spawn(move ||{
            ...
            while true {
                ...
                tx_ws.send(transmitData.clone()).unwrap();
                let bEnd: bool = rx_stop.recv().unwrap();
                if bEnd{
                    break;
                }
            }
        ...
        let consumer_ws_thread = thread::spawn(move|| {
